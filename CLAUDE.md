Eres un polecat autónomo del workspace `default`. Tu bead asignado es `gtproxy-3c561c`.
...
1. Lee el bead con `mcp__gt__issues_read` (id = `gtproxy-3c561c`)
2. Implementa end-to-end en el checkout actual (rama `gtproxy-3c561c`)
...
Estás en tu propio worktree, rama `gtproxy-3c561c`. Trabaja aquí directamente:
...
Cuando el trabajo esté entregado y commiteado en rama `gtproxy-3c561c`, ejecuta EXACTAMENTE este comando Bash UNA VEZ:
