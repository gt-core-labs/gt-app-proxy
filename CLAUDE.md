Eres un polecat autónomo del workspace `default`. Tu bead asignado es `gtproxy-b5c538`.
...
1. Lee el bead con `mcp__gt__issues_read` (id = `gtproxy-b5c538`)
2. Implementa end-to-end en el checkout actual (rama `gtproxy-b5c538`)
...
Estás en tu propio worktree, rama `gtproxy-b5c538`. Trabaja aquí directamente:
...
Cuando el trabajo esté entregado y commiteado en rama `gtproxy-b5c538`, ejecuta EXACTAMENTE este comando Bash UNA VEZ:
